...
public interface StudentRepository extends JpaRepository<Student, Long> {
	...
	@Query("select (count(s) > 0) from Student s where s.gradeLevel.gradeName = ?1")
	boolean isGradeNameExist(String gradeName);

	@Query("select s from Student s where s.section.sectionId = ?1")
	Iterable<Student> getStudentBySectionId(Integer sectionId);

	@Query("select count(s) from Student s where s.section.sectionId = ?1")
